      ...
      <div>
        <Card  title={<p className="moduleTitle"><Icon type="tag-o" /> 标签</p>} >
            ...
            {this.state.tagList.map((item,index)=> {
              ...
              return ( <Tag   onClick={()=>{this.props.toTag(item.tid,item.label)}} key={index} style={{marginTop:10,marginLeft:5,borderColor:randomColor,color:randomColor }} >{item.label}</Tag> )
            })}
